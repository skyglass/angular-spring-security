...
(function() {
    ...
    var templateUrl = 'js/directives/breadcrumbs/uiBreadcrumbs.tpl.html';
    ...
    module.directive('uiBreadcrumbs', ['$interpolate', '$state', function($interpolate, $state) {
            return {
                ...
                link: function(scope) {
                    ...
                    function updateBreadcrumbsArray() {
                        ...
                        while(currentState && currentState.name !== '') {
                            ...
                            if (workingState) {
                                ...
                                route = getRouteName(workingState);
                    ...
                    /**
                     * Resolve the route of the specified state. Take the property specified by the `proxyLink-property`
                     * attribute and look up the corresponding property on the state's config object. The specified string can be interpolated against any resolved
                     * properties on the state config object, by using the usual {{ }} syntax.
                     * @param currentState
                     * @returns {*}
                     */                    
                    function getRouteName(currentState) {
                        var interpolationContext;
                        var propertyReference;
                        var routeName;

                        if (!scope.proxyLinkProperty) {
                            // if the proxyLink-property attribute was not specified, default to the state's name
                            return currentState.name;
                        }
                        propertyReference = getObjectValue(scope.proxyLinkProperty, currentState);
                        ...
                        if (propertyReference === false) {
                            ...
                            // use the $interpolate service to handle any bindings in the propertyReference string.
                            interpolationContext =  (typeof currentState.locals !== 'undefined') ? currentState.locals.globals : currentState;
                            routeName = $interpolate(propertyReference)(interpolationContext);
                            return routeName;
                        }
                    }                    
